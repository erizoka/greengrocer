...
class CartController extends GetxController {
  ...
  Future<bool> changeItemQuantity({
    ...
    if (result) {
      if (quantity == 0) {
        ...
        cartItems.firstWhere((c) => c.id == item.id).quantity = quantity;
      }

      update();
    } else {
      _utilsServices.showToast(
        msg: 'Ocorreu um erro ao alterar a quantidade do produto',
        isError: true,
      );
    }
  ...
  Future<void> getCartItems() async {
    final CartResult<List<CartItemModel>> result = await _cartRepository
        .gatCartItems(
          token: _authController.user.token!,
  ...
  int getItemIndex(ItemModel item) {
    return cartItems.indexWhere((i) => i.item.id == item.id);
  }
